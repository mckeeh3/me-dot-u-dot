...
public class GameMoveTools {
  ...
  @FunctionTool(description = """
      ...
      """)
  public GetGameMoveLogsTool.Response getGameMoveLogs(
    ...
    var response = GetGameMoveLogsTool.Response.from(gameId, agentId, logs);
  ...
  static String json(GetGameMoveLogsTool.Response response) {
    var om = JsonSupport.getObjectMapper();
    try {
      return om.writerWithDefaultPrettyPrinter().writeValueAsString(response);
    } catch (JsonProcessingException e) {
  ...
  public interface GetGameMoveLogsTool {
    ...
    record MoveResponse(int moveNumber, String response) {
